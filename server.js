...
app.get("/getTimesheet", (req, res) => {
  ...
  connection.query(query, [userId, Week], (err, results) => {
    if (err) {
...
app.post("/updateTimesheet", (req, res) => {
  ...
  const query = `
    ...
    WHERE UserID = ? AND Week = ? AND ProjectName = ?
  `;

  // Execute the query for each project
  const projects = [
    { name: "Project A", dates: projectA },
    { name: "Project B", dates: projectB },
    { name: "Leave", dates: leave },
  ];

  projects.forEach((project) => {
    connection.query(query, [...project.dates, userID, week, project.name], (err, result) => {
      if (err) {
